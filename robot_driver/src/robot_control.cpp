...
#include "/home/plane/1/src/robot_driver/include/robot_control.h"
...
kRobot::Robot()
{
    ...
    //实例化订阅者对象
    cmd_vel_sub_ = nh_.subscribe<geometry_msgs::Twist>(cmd_vel_topic_, 100, &Robot::cmdVelCallback, this);
    //数据初始化
    memset(&pos_data_, 0, sizeof(pos_data_));

    //提示信息，串口端口号和波特率
    ROS_INFO(" Robot Set serial %s at %d baud", serial_port_.c_str(), serial_port_baud_);
    ...
    if(openSerialPort())
    {
        ...
        {
            //启动串口接收线程
            boost::thread recvSerial_thread(boost::bind(&Robot::recvCallback,this));
        }
        catch(...)
        {
            ROS_INFO(" Robot can not open recvSerial_thread, Please check the serial port cable ");
    ...
    ROS_INFO(" Robot is Connected to OpenCRP board ");



    //提示信息，发布订阅的话题消息
    ROS_INFO(" Robot setup publisher  on odom [nav_msgs/Odometry]");
    ROS_INFO(" Robot setup publisher  on imu [sensor_msgs/Imu]");
    ROS_INFO(" Robot setup publisher  on bat_vol [float]");
    ROS_INFO(" Robot setup subscriber on cmd_vel [geometry_msgs/Twist]");

    //机器人启动完成提示信息
    ROS_INFO(" Robot initialization completed, is Running!");
...
{ 
    ...
    //提示信息
    ROS_INFO(" Robot shutting down."); 
}

/*
 * @功  能  初始化串口
 */
bool obot::openSerialPort()
{
    //检查串口是否已经被打开
    if(serial_ptr_)
    {
 ...
 * @功  能  串口接收数据处理
 */
void Robot::recvDataHandle(uint8_t* buffer_data)
{
    //机器人数据
 ...
 * @返回值	 无
 */
void Robot::sendSerialPacket(uint8_t *pbuf, uint8_t len, uint8_t num)
{
	uint8_t i,cnt;	
 ...
 * @功  能  计算IMU四元数
 */
void Robot::calculateImuQuaternion(struct imu_data imu_cel)
{
    float recipNorm;
